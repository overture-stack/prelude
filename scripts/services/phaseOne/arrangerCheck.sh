...
until curl -s -o /dev/null -w "%{http_code}" "$ARRANGER_FILE_DATA_URL" | tr -d '/' | grep -q "200"; do
...
until curl -s -o /dev/null -w "%{http_code}" "$ARRANGER_TABULAR_DATA_URL" | tr -d '/' | grep -q "200"; do
    printf "Trying again in %d seconds...\n" "$RETRY_DELAY"
    sleep "$RETRY_DELAY"
done

...
retry_count=0
until curl -s -o /dev/null -w "%{http_code}" "$ARRANGER_FILE_DATA_URL" | tr -d '/' | grep -q "200" || [ $retry_count -ge $MAX_RETRIES ]; do
    retry_count=$((retry_count + 1))
    if [ $retry_count -ge $MAX_RETRIES ]; then
        echo -e "\033[1;31mError:\033[0m File Data Arranger is not reachable after $MAX_RETRIES attempts"
        show_troubleshooting_tips "File Data Arranger"
        exit 1
    fi

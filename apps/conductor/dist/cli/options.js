...
exports.parseCommandLineArgs = exports.configureCommandOptions = void 0;
...
function configureCommandOptions(program) {
    ...
    program
        ...
        .option("-u, --lectern-url <url>", "Lectern server URL", process.env.LECTERN_URL || "http://localhost:3031")
    ...
    program
        ...
        .option("-u, --lyric-url <url>", "Lyric server URL", process.env.LYRIC_URL || "http://localhost:3030")
    ...
    program
        ...
        .option("-u, --lyric-url <url>", "Lyric server URL", process.env.LYRIC_URL || "http://localhost:3030")
        .option("-l, --lectern-url <url>", "Lectern server URL", process.env.LECTERN_URL || "http://localhost:3031")
        .option("-d, --data-directory <path>", "Directory containing CSV data files", process.env.LYRIC_DATA)
        .option("-c, --category-id <id>", "Category ID", process.env.CATEGORY_ID || "1")
        .option("-g, --organization <name>", "Organization name", process.env.ORGANIZATION || "OICR")
        .option("-m, --max-retries <number>", "Maximum number of retry attempts", process.env.MAX_RETRIES || "10")
        .option("-r, --retry-delay <milliseconds>", "Delay between retry attempts in milliseconds", process.env.RETRY_DELAY || "1000")
        .option("-o, --output <path>", "Output directory for response logs")
        .option("--force", "Force overwrite of existing files")
        .action(() => {
        /* Handled by main.ts */
    ...
    program
        ...
        .option("--index-url <url>", "Indexing service URL", process.env.INDEX_URL || "http://localhost:11235")
        .option("--repository-code <code>", "Repository code to index", process.env.REPOSITORY_CODE)
        .option("--organization <name>", "Organization name filter", process.env.ORGANIZATION)
    ...
    program
        ...
        .option("-s, --schema-file <path>", "Schema JSON file to upload")
        .option("-u, --song-url <url>", "SONG server URL", process.env.SONG_URL || "http://localhost:8080")
        .option("-t, --auth-token <token>", "Authentication token", process.env.AUTH_TOKEN || "123")
        .option("-o, --output <path>", "Output directory for response logs")
        .option("--force", "Force overwrite of existing files")
        .action(() => {
        /* Handled by main.ts */
    ...
    program
        ...
        .option("--name <name>", "Study display name (defaults to study ID)", process.env.STUDY_NAME)
        .option("-g, --organization <name>", "Organization name", process.env.ORGANIZATION || "OICR")
        .option("--description <text>", "Study description", process.env.DESCRIPTION || "string")
        .option("-t, --auth-token <token>", "Authentication token", process.env.AUTH_TOKEN || "123")
        .option("-o, --output <path>", "Output directory for response logs")
    ...
    program
        ...
        .option("-u, --song-url <url>", "SONG server URL", process.env.SONG_URL || "http://localhost:8080")
        .option("-s, --score-url <url>", "Score server URL", process.env.SCORE_URL || "http://localhost:8087")
        ...
        .option("-d, --data-dir <path>", "Directory containing data files", process.env.DATA_DIR || "./data")
        .option("--output-dir <path>", "Directory for manifest file output", process.env.OUTPUT_DIR || "./output")
        .option("-m, --manifest-file <path>", "Path for manifest file", process.env.MANIFEST_FILE)
        .option("-t, --auth-token <token>", "Authentication token", process.env.AUTH_TOKEN || "123")
        .option("--ignore-undefined-md5", "Ignore files with undefined MD5 checksums", false)
        .option("-o, --output <path>", "Output directory for response logs")
        .option("--force", "Force studyId from command line instead of from file", false)
        .action(() => {
        /* Handled by main.ts */
    });
    program
    ...
    try {
        ...
        if (profile === constants_1.Profiles.LYRIC_DATA) {
            ...
            if (positionalArgs.length > 0 && !options.dataDirectory) {
                ...
                logger_1.Logger.debug `Captured data directory from positional argument: ${options.dataDirectory}`;
            }
        }
        ...
        logger_1.Logger.debug `Parsed profile: ${profile}`;
        logger_1.Logger.debug `Parsed file paths: ${filePaths.join(", ")}`;
        ...
        if (isNaN(batchSize) || batchSize <= 0) {
            throw errors_1.ErrorFactory.validation("Invalid batch size", { batchSize: options.batchSize }, ["Batch size must be a positive number", "Example: --batch-size 1000"]);
        }
        if (isNaN(maxRetries) || maxRetries < 0) {
            throw errors_1.ErrorFactory.validation("Invalid max retries", { maxRetries: options.maxRetries }, [
                "Max retries must be a non-negative number",
                "Example: --max-retries 3",
            ]);
        }
        if (isNaN(retryDelay) || retryDelay < 0) {
            throw errors_1.ErrorFactory.validation("Invalid retry delay", { retryDelay: options.retryDelay }, [
        ...
        const dbHostPort = parseHostPort(options.dbHost || process.env.DB_HOST || "localhost:5435", "localhost", "5435");
        ...
        const config = {
            ...
            song: {
                ...
                organization: options.organization || process.env.ORGANIZATION || "string",
                ...
                allowDuplicates: options.allowDuplicates ||
                    ...
                    false,
                ignoreUndefinedMd5: options.ignoreUndefinedMd5 ||
                    process.env.IGNORE_UNDEFINED_MD5 === "true" ||
                    false,
                // Combined Score functionality (now part of song config)
                scoreUrl: options.scoreUrl || process.env.SCORE_URL || "http://localhost:8087",
            ...
            maestroIndex: {
                url: options.indexUrl || process.env.INDEX_URL || "http://localhost:11235",
        ...
        throw errors_1.ErrorFactory.parsing("Failed to parse command line arguments", { options, originalError: error }, [

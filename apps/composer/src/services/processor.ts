...
export async function processCSVFile(filePath: string, config: Config, client: Client) {
  ...
  try {
      ...
      try {
        ...
        if (isFirstLine) {
          ...
          process.stdout.write(
            `${chalk.blue.bold('📊 Total records to process:')} ${chalk.yellow.bold(
              totalLines.toString()
            )}\n\n`
          );
        ...
        if (batchedRecords.length >= config.batchSize) {
          await sendBulkWriteRequest(client, batchedRecords, config.elasticsearch.index, count => {
            failedRecords += count;
          });
    ...
    if (error instanceof Error) {
      ...
      process.stdout.write(
        `└─ Processing stage: ${isFirstLine ? 'Header validation' : 'Data processing'}\n`
      );
    }

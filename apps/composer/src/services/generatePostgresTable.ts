...
export function generatePostgresTable(
  ...
  try {
    ...
    headers.forEach((header, index) => {
      ...
      // Check for potential primary key (ID columns)
      const isPotentialPK =
        sanitizedHeader.includes("id") &&
        sanitizedHeader.length <= 10 &&
        index === 0; // Usually first column

      let columnDef = `  ${sanitizedHeader} ${dataType}`;

      // Add NOT NULL for non-empty required-looking columns
      const nonEmptyCount = sampleValues.filter(
        (v) => v && v.trim() !== ""
      ).length;
      const totalCount = sampleValues.length;
      const fillRate = totalCount > 0 ? nonEmptyCount / totalCount : 0;

      if (isPotentialPK || fillRate > 0.95) {
        columnDef += " NOT NULL";
      }
      ...
      if (options.includeConstraints && isPotentialPK) {
        constraints.push(
          `  CONSTRAINT pk_${sanitizedTableName} PRIMARY KEY (${sanitizedHeader})`
        );
      }
      ...
      if (options.includeIndexes) {
        if (
          sanitizedHeader.includes("id") ||
          sanitizedHeader.includes("date")
        ) {
          indexes.push(
            `CREATE INDEX IF NOT EXISTS idx_${sanitizedTableName}_${sanitizedHeader} ON ${
              options.schema ? options.schema + "." : ""
            }${sanitizedTableName} (${sanitizedHeader});`
          );
        }
      }
    });
    ...
    const fullTableName = options.schema
      ? `${options.schema}.${sanitizedTableName}`
      : sanitizedTableName;
    ...
    if (options.schema) {
      sql += `CREATE SCHEMA IF NOT EXISTS ${options.schema};\n\n`;
    }

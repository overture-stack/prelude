...
describe("parseCSVLine", () => {
  ...
  describe("header row mode", () => {
    ...
    it("returns empty array for a blank header line", () => {
      const result = parseCSVLine("", ",", true);
      expect(result).toEqual([]);
    });
  });

  describe("error handling", () => {
    it("throws ComposerError for malformed CSV", () => {
      expect(() => parseCSVLine('"unclosed quote', ",")).toThrow(ComposerError);
    });
  });
});

...
export class PostgresCommand extends Command {
  ...
  constructor() {
    super("PostgreSQL Table", path.join("generatedConfigs", "postgresConfigs"));
  ...
  protected isUsingDefaultPath(cliOutput: CLIOutput): boolean {
    return (
      cliOutput.outputPath ===
        path.join("generatedConfigs", "postgresConfigs", "create_table.sql") ||
      cliOutput.outputPath ===
        path.join("generatedConfigs", "postgresConfigs") ||
  ...
  protected async validate(cliOutput: CLIOutput): Promise<void> {
    ...
    if (!cliOutput.outputPath) {
      throw ErrorFactory.args("Output path is required", [
        "Use -o or --output to specify where to save the SQL file",
        "Example: -o create_patients_table.sql",
        "The output will be a PostgreSQL CREATE TABLE statement",
      ]);
    }
  ...
  protected async execute(cliOutput: CLIOutput): Promise<any> {
    ...
    try {
      ...
      const sqlStatement = generatePostgresTable(
        ...
        sampleData,
        {
          includeConstraints: postgresConfig?.includeConstraints || false,
          includeIndexes: postgresConfig?.includeIndexes || false,

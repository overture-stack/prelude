...
exports.generatePostgresTable = generatePostgresTable;
...
function isValidTimestamp(timestampString) {
    ...
    const matchesFormat = timestampFormats.some((format) => format.test(timestampString));
    if (!matchesFormat)
        return false;
...
function inferPostgresType(columnName, sampleValues, rules = defaultRules) {
    logger_1.Logger.debug `Inferring PostgreSQL type for column: ${columnName}`;
    // Filter out empty values for analysis
    const nonEmptyValues = sampleValues.filter((v) => v && v.trim() !== "");
    if (nonEmptyValues.length === 0) {
        logger_1.Logger.debugString("All values are empty, defaulting to VARCHAR");
    ...
    if (numericAnalysis.isAllNumeric) {
        if (numericAnalysis.isAllInteger) {
            ...
            if (numericAnalysis.maxValue <= 32767 &&
                numericAnalysis.minValue >= -32768) {
                logger_1.Logger.debugString("Detected SMALLINT type");
                return "SMALLINT";
            }
            else if (numericAnalysis.maxValue <= rules.integerThreshold &&
                numericAnalysis.minValue >= -rules.integerThreshold - 1) {
                logger_1.Logger.debugString("Detected INTEGER type");
                return "INTEGER";
            }
            else {
                logger_1.Logger.debugString("Detected BIGINT type");
                return "BIGINT";
            }
        }
        else {
            // Decimal/numeric type
            const precision = Math.max(rules.decimalPrecision, numericAnalysis.maxDecimalPlaces + 5);
            const scale = Math.max(rules.decimalScale, numericAnalysis.maxDecimalPlaces);
            logger_1.Logger.debugString(`Detected DECIMAL type with precision ${precision}, scale ${scale}`);
            return `DECIMAL(${precision},${scale})`;
        }
    }
    // Check for date/timestamp types based on column name and sample values
    const hasDatePattern = rules.datePatterns.some((pattern) => lowerColumnName.includes(pattern));
    const hasTimestampPattern = rules.timestampPatterns.some((pattern) => lowerColumnName.includes(pattern));
    if (hasTimestampPattern || nonEmptyValues.some((v) => isValidTimestamp(v))) {
        logger_1.Logger.debugString("Detected TIMESTAMP type");
        return "TIMESTAMP";
    }
    ...
    if (maxLength > rules.maxVarcharLength) {
        logger_1.Logger.debugString("Detected TEXT type (long strings)");
        return "TEXT";
    }
    // Use VARCHAR with appropriate length
    const varcharLength = Math.max(maxLength * 1.5, 50); // Add 50% buffer, minimum 50
    const finalLength = Math.min(Math.ceil(varcharLength), rules.maxVarcharLength);
    logger_1.Logger.debugString(`Detected VARCHAR(${finalLength}) type`);
 ...
 */
function generatePostgresTable(tableName, headers, sampleData, options = {}) {
    try {
        logger_1.Logger.debugString("Generating PostgreSQL CREATE TABLE statement");
        logger_1.Logger.debug `Table name: ${tableName}`;
        logger_1.Logger.debug `Processing ${headers.length} columns`;
        const rules = {
            ...defaultRules,
            ...options.customRules,
        };
        ...
        if (sanitizedTableName !== tableName) {
            logger_1.Logger.warn `Table name "${tableName}" sanitized to "${sanitizedTableName}"`;
        }
        ...
        headers.forEach((header, index) => {
            ...
            if (sanitizedHeader !== header) {
                logger_1.Logger.debug `Column "${header}" sanitized to "${sanitizedHeader}"`;
            }
            // Infer data type
        ...
        sql += `-- Sample data analysis: ${Math.max(...Object.values(sampleData).map((arr) => arr.length))} rows\n`;
        ...
        logger_1.Logger.debugString("PostgreSQL CREATE TABLE statement generated successfully");
        return sql;
    }
    catch (error) {
        logger_1.Logger.errorString("Error generating PostgreSQL table");
        logger_1.Logger.debugObject("Error details", { tableName, headers, error });
        throw error;
    }
}
//# sourceMappingURL=generatePostgresTable.js.map
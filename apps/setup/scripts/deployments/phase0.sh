...
main() {
    ...
    if ! version_check "$DOCKER_ENGINE_MIN_VERSION" "$ENGINE_VERSION"; then
        printf "   └─ \033[1;31mError:\033[0m Docker Engine version %s does not meet minimum requirement of %s\n" "$ENGINE_VERSION" "$DOCKER_ENGINE_MIN_VERSION"
        printf "      \033[1;33mHow to update Docker:\033[0m\n"
    ...
    printf "   └─ \033[1;36mInfo:\033[0m Docker Engine version %s detected\n" "$ENGINE_VERSION"

    # Check Docker is running
    if ! docker info >/dev/null 2>&1; then
        printf "   └─ \033[1;31mError:\033[0m Docker daemon is not running\n"
        exit 1
    fi
    printf "   └─ \033[1;36mInfo:\033[0m Docker daemon is running\n"
    ...
    if ! safe_compare -lt "$CPU_CORES" "$DOCKER_MIN_CPU_CORES"; then
        printf "   └─ \033[1;36mInfo:\033[0m Docker CPU allocation meets minimum requirements (%s cores)\n" "$CPU_CORES"
    else
        printf "   └─ \033[1;33m⚠ Warning:\033[0m Docker CPU cores (%s) are less than recommended (%s)\n" "$CPU_CORES" "$DOCKER_MIN_CPU_CORES"
        printf "      \033[1;33mRecommendation:\033[0m Please increase Docker CPU allocation to %s cores for Docker.\n" "$DOCKER_MIN_CPU_CORES"
        ...
        case "$response" in
            [yY]|[yY][eE][sS])
                ...
                ;;
            *)
                printf "      Deployment cancelled. Please increase Docker CPU allocation and try again.\n"
                exit 1
                ;;
        esac
    fi
    ...
    if ! safe_compare -lt "$MEMORY_GB" "$DOCKER_MIN_MEMORY_GB"; then
        printf "   └─ \033[1;36mInfo:\033[0m Docker memory allocation meets minimum requirements (%s GB)\n" "$MEMORY_GB"
    else
        printf "   └─ \033[1;33m⚠ Warning:\033[0m Docker memory (%s GB) is less than recommended (%s GB)\n" "$MEMORY_GB" "$DOCKER_MIN_MEMORY_GB"
        printf "      \033[1;33mRecommendation:\033[0m Please increase Docker memory allocation to %s GB for Docker.\n" "$DOCKER_MIN_MEMORY_GB"
        ...
        case "$response" in
            [yY]|[yY][eE][sS])
                ...
                ;;
            *)
                printf "      Deployment cancelled. Please increase Docker memory allocation and try again.\n"
                exit 1
                ;;
        esac
    fi
    ...
    if ! safe_compare -lt "$DISK_GB" "$DOCKER_MIN_DISK_GB"; then
        ...
        printf "   └─ \033[1;33m⚠ Warning:\033[0m Docker virtual disk space (%s GB) is less than recommended (%s GB)\n" "$DISK_GB" "$DOCKER_MIN_DISK_GB"
        printf "      \033[1;33mRecommendation:\033[0m Please increase Docker virtual disk allocation to %s GB for Docker.\n" "$DOCKER_MIN_DISK_GB"
        ...
        case "$response" in
            [yY]|[yY][eE][sS])
                ...
                ;;
            *)
                printf "      Deployment cancelled. Please increase Docker disk allocation and try again.\n"
                exit 1
                ;;
        esac
    fi
    ...
    # Success message
    printf "\n\033[1;32m✓ Ready to deploy\033[0m\n"
}

...
# File: /setup/scripts/services/postgres/setup_postgres.sh
...
SQL_CONFIGS_DIR=${POSTGRES_CONFIGS_DIR:-"./apps/setup/configs/postgresConfigs"}
...
printf "   └─ \033[1;36mSetup:\033[0m Checking if PostgreSQL is available\n"
...
until PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT 1;" > /dev/null 2>&1; do
   ...
   if [ "$RETRY_COUNT" -ge "$MAX_RETRIES" ]; then
       printf "   └─ \033[1;31mError:\033[0m Failed to connect after %d attempts\n" "$MAX_RETRIES"
       exit 1
   fi
...
TABLE_CHECK_SQL="SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'setup_sql_executed');"
...
if ! PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "$TABLE_CHECK_SQL" | grep -q "t"; then
    ...
    PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "
        CREATE TABLE IF NOT EXISTS setup_sql_executed (
...
echo "$SQL_FILES" | while IFS= read -r sql_file; do
    ...
    EXECUTION_CHECK="SELECT COUNT(*) FROM setup_sql_executed WHERE sql_name = '$sql_name' AND file_hash = '$FILE_HASH';"
    ...
    if [ "$ALREADY_EXECUTED" = "0" ]; then
        ...
        if PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f "$sql_file" > /dev/null 2>&1; then
            ...
            PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "
                INSERT INTO setup_sql_executed (sql_name, file_path, file_hash)

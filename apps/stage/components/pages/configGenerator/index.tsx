			...
			>
				...
				<section css={sectionStyle}>
					...
					<div css={css`display: flex; gap: 12px; margin-bottom: 16px; align-items: center;`}>
						...
						{filename && (
							<span css={css`font-size: 13px; color: ${theme.colors.grey_5};`}>
								{filename}
							</span>
						)}
						<input
				...
				{/* Step 3 – Output */}
				{configs && (
					<section css={css`background: ${theme.colors.white}; border: 1px solid ${theme.colors.grey_2}; border-radius: 5px; overflow: hidden;`}>
						<h2 css={css`${sectionHeadingStyle}; padding: 20px 24px 0;`}>3. Generated Configs</h2>

						<div css={css`display: flex; border-bottom: 1px solid ${theme.colors.grey_2}; overflow-x: auto; padding: 0 24px;`}>
							{OUTPUT_TABS.map(({ key, label }) => (
								<button
									...
									css={css`
										...
										font-weight: ${activeTab === key ? 'bold' : 'normal'};
										margin-bottom: -1px;
									`}
								>
									{label}

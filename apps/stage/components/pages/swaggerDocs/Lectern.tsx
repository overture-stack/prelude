...
const Lectern = () => {
	...
	useEffect(() => {
		const fetchSwaggerSpec = async () => {
			try {
				// Try to fetch directly from Lectern service first
				const response = await fetch('/api/lectern/api-docs'); // TODO update lectern or find url with spec
	...
	const swaggerConfig: SwaggerUIProps = {
		...
		requestInterceptor: (req) => {
			...
			if (!req.url.startsWith('/api/lectern')) {

...
> **ILLUSTRATION NEEDED:** A horizontal data flow diagram showing the journey of data through the system. From left to right: (1) CSV file → (2) Conductor (transform + load) → (3) Elasticsearch (index + store) → (4) Arranger (GraphQL API + UI config) → (5) Stage (browser portal). Include arrows showing the direction of data flow. Below the main flow, show Composer generating config files that feed into Elasticsearch (mappings) and Arranger (UI configs).
...
The components used in this workshop are part of the broader [Overture](https://overture.bio) open-source platform for genomic data management. The search and exploration stack we're using can be extended with additional services:

![Platform Integration](/docs/images/platformintegration.png)
